...
export const createTestUser = async (dbPool, userData) => {
    const bcrypt = require('bcryptjs');
    const hashedPassword = await bcrypt.hash(userData.password, 10);
    const result = await dbPool.query(`INSERT INTO users (id, email, password_hash, role, employee_id, created_at, updated_at) 
     VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), NOW()) 
     RETURNING id, email, role, employee_id`, [userData.email, hashedPassword, userData.role, userData.employeeId]);
    return result.rows[0];
};
export const createTestEmployee = async (dbPool, employeeData) => {
    const result = await dbPool.query(`INSERT INTO employees (
