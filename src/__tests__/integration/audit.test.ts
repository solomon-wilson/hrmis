...
ual(5rThanOrEqeGreate.count)).toBns[0]eateActioeInt(crpect(pars ex;
     l(1)nOrEquaThaoBeGreaterength).tateActions.l(crexpect');
      e= 'CREATEow.action == rlter(row =>s.fiats.rowauditSttions = t createAc cons    
     al(1);
  terThanOrEquBeGreah).tos.rows.lengtt(auditStat  expec `);
  ...
  VAL NTERW() - I= NOd_at >meforer    WHERE p 
   ...
   d_at) aorme', perf'dayDATE_TRUNC(       t,
      ...
      ct      aLECT 
        ...
        SEry(`ool.queit dbPats = awatStonst audi c    tatistics
 it sry aud   // Que  
      }
...
24te: '20tartDa      s   neering',
 ent: 'Engi departm    er',
  ...
  pany.com',omter@c: 'user.filmail    e
   ...
   ame: 'UserstN     fi
     ({nd        .senToken}`)
rer ${hrAdmiion', `Beaathoriz  .set('Aut   es')
   i/employe  .post('/ap
      uest(app)ait reqponse = awst createReson ce
     yee emploCreat     //  () => {
 ', asyncertering by us log filrt auditsuppold est('shou;

    t);
    })TE'.toBe('CREA].action)Logs.rows[0(recentect    expngth(1);
  .toHaveLetLogs.rows)ent(rec expec  ;

   ]
      )String()oISOurFromNow.ting(), oneHotrtoISOS oneHourAgo.employeeId,     [D $3',
   $2 ANt BETWEEN _aformed= $1 AND perid ERE entity_udit_logs WH * FROM a   'SELECT    
 ery(ool.qubPwait d atLogs = recen constge
     rane n datt logs withi audi// Query      0);

00* 1* 60 0 ) + 6.getTime(w Date(now neFromNow = oneHour      const0 * 1000);
- 60 * 6me() getTiw.Date(norAgo = new t oneHou cons
     ();ew Datenow = nconst ing
       for filternt timeret cur
      // Ge.id;
bodyesponse.Id = createRt employee  cons       });

   
  TIME'ype: 'FULL_employmentT        ',
  01-01te: '2024-rtDa     sta
     eering',ginartment: 'En  dep',
        gineerEn: '    jobTitle
      m',any.co.filter@compl: 'date      emai
    ter', 'File:Namst la      ,
   e: 'Date'tNam   firs     {
    .send(n}`)
      hrAdminToke ${Bearerrization', `set('Autho
        .yees')lost('/api/emp       .poest(app)
 = await requponse escreateRconst 
      ployeeeate em // Cr     
sync () => { range', ang by date log filteriaudit support ld'shouest(   t
  => {, ()Reporting'and uerying dit Log Q('Au  describe);

;
  }})
    
      });f(Date);ceOoBeInstanrformedAt).tt(log.pe       expec();
 .toBeDefinedBy)medfor.per(logpect ex   
    BeDefined();.to.changes)og    expect(l);
    HANGE)$/STATUS_CTE|UPDATE|ch(/^(CREA).toMationog.actexpect(l  
      loyeeId);).toBe(emp.entityIdexpect(log        LOYEE');
oBe('EMPtyType).tog.entiect(l        expned();
d).toBeDefig.i expect(lo      g => {
 .forEach(lo  auditLogss
    quired fieldhas refy each log      // Veri
 stamps);
Timeual(sortedamps).toEqxpect(timestder
      ecending or Desb - a); // => .sort((a, b)mestamps][...ti = psstamTimeedortconst s
      ));).getTime(performedAtte(log. Da => newogs.map(logmps = auditLtaconst times   order
   al chronologic Verify  //NGE

     S_CHATATUPDATEs + S 2 UTE +); // CREAHaveLength(4).toogsct(auditL   expeeId);
   tory(employentityHistE.geRepowait auditogs = aauditLst     conail
   audit trify complete     // Ver;

 
        })Vacation'son: '        rea
  1',: '2024-02-0ate effectiveD         AVE',
atus: 'ON_LE          stend({

        .sminToken}`)Adhrearer ${ `Bization',.set('Author      tus`)
  oyeeId}/stas/${emplmployeeput(`/api/e)
        . request(appait
      awtatusange s  // Ch });

    1-555-5555'phone: '+nd({       .seoken}`)
  ${hrAdminT `Bearer rization',set('Autho   .  )
   ployeeId}`ployees/${em`/api/em    .put(
    equest(app)    await r});

  neer' ngiior Ee: 'Sentl jobTiend({    .s  
  dminToken}`) ${hrAarertion', `Behoriza'Aut      .set(eeId}`)
  {employployees/$(`/api/em.put        quest(app)
await re
      mes titipleyee mullote empda// Up    

  .id;onse.bodyespteRread = cemployeeIt      cons});

   ME'
      FULL_TItType: '   employmen     1-01',
  : '2024-0 startDate
         ing',gineertment: 'En       deparineer',
   bTitle: 'Engjo  ',
        @company.comrity.test 'integemail:    t',
      tName: 'Tes      lasty',
    e: 'Integriam firstN     {
      .send(
      n}`){hrAdminTokearer $ation', `Behoriz .set('Aut   
    ees')/api/employ   .post('pp)
     it request(awaesponse = at createR consns
     operatiom multiple for   // Per   > {
ync () = asrations',ss operity acro integudit logintain ashould ma
    test('
}
    });     ined();
 r).toBeDefrroct(e      expeed
  protecty roperle plogs arf audit xpected is is e  // Thi
      rror) {catch (e    }  });
  ruered: tt({ tampeecbjot.toMatchOanges).n.ch[0]updatedLogs     expect(Id);
   oyeeistory(empltEntityHauditRepo.geait gs = awt updatedLo     cons intact
   a is still datinalthe origjust verify t, we'll 