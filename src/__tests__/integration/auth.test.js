...
describe('Authentication Flow Integration Tests', () => {
    ...
    describe('User Registration and Login', () => {
        ...
        test('should login with valid credentials', async () => {
            ...
            await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())', ['login@company.com', hashedPassword, 'EMPLOYEE']);
            const response = await request(app)
                .post('/api/auth/login')
                .send({
        ...
        test('should reject invalid credentials', async () => {
            ...
            await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())', ['invalid@company.com', hashedPassword, 'EMPLOYEE']);
            await request(app)
                .post('/api/auth/login')
                .send({
    ...
    describe('Token Management', () => {
        ...
        beforeEach(async () => {
            ...
            const hashedPassword = await bcrypt.hash('password123', 10);
            const userResult = await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW()) RETURNING id', ['token@company.com', hashedPassword, 'EMPLOYEE']);
        ...
        test('should reject access with expired token', async () => {
            ...
            const expiredToken = jwt.sign({ userId, email: 'token@company.com', role: 'EMPLOYEE' }, process.env.JWT_SECRET, { expiresIn: '-1h' } // Expired 1 hour ago
            );
            await request(app)
                .get('/api/employees/self')
    ...
    describe('Role-Based Access Control', () => {
        ...
        beforeEach(async () => {
            ...
            for (const user of users) {
                const hashedPassword = await bcrypt.hash('password123', 10);
                await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())', [user.email, hashedPassword, user.role]);
                const loginResponse = await request(app)
                    .post('/api/auth/login')
                    .send({
                    email: user.email,
    ...
    describe('Session Management', () => {
        test('should handle multiple concurrent sessions', async () => {
            // Create user
            const hashedPassword = await bcrypt.hash('password123', 10);
            await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())', ['multi@company.com', hashedPassword, 'EMPLOYEE']);
        ...
        test('should handle session timeout gracefully', async () => {
            // Create user
            const hashedPassword = await bcrypt.hash('password123', 10);
            const userResult = await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW()) RETURNING id', ['timeout@company.com', hashedPassword, 'EMPLOYEE']);
            // Create a token with very short expiration
            const shortLivedToken = jwt.sign({ userId: userResult.rows[0].id, email: 'timeout@company.com', role: 'EMPLOYEE' }, process.env.JWT_SECRET, { expiresIn: '1ms' } // Expires immediately
            );
    ...
    describe('Security Features', () => {
        test('should rate limit login attempts', async () => {
            ...
            const attempts = Array.from({ length: 6 }, () => request(app)
                .post('/api/auth/login')
                .send(userData));
        ...
        test('should sanitize sensitive data in responses', async () => {
            // Create user
            const hashedPassword = await bcrypt.hash('password123', 10);
            await dbPool.query('INSERT INTO users (id, email, password_hash, role, created_at, updated_at) VALUES (gen_random_uuid(), $1, $2, $3, NOW(), NOW())', ['sanitize@company.com', hashedPassword, 'EMPLOYEE']);
            const response = await request(app)
                .post('/api/auth/login')
                .send({
