...
describe('ExportService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        mockFs = fs;
        mockAuditLogRepository = new AuditLogRepository();
        // Mock fs methods
        ...
        exportService.auditLogRepository = mockAuditLogRepository;
    });
    describe('exportEmployeeRoster', () => {
        const csvOptions = { format: 'CSV' };
        ...
        it('should export employee roster to CSV for HR admin', async () => {
            ...
            mockAuditLogRepository.logDataExport.mockResolvedValue({});
            const result = await exportService.exportEmployeeRoster(mockEmployeeRosterReport, csvOptions, mockHRAdminContext);
            expect(result.format).toBe('CSV');
            expect(result.recordCount).toBe(2);
            expect(result.generatedBy).toBe(mockHRAdminContext.userId);
            expect(mockCsvWriter.writeRecords).toHaveBeenCalledWith(expect.arrayContaining([
            ...
            expect(mockAuditLogRepository.logDataExport).toHaveBeenCalledWith('EMPLOYEE_ROSTER', expect.objectContaining({ format: 'CSV', recordCount: 2 }), mockHRAdminContext.userId, expect.objectContaining({ action: 'csv_export' }));
        });
        it('should export employee roster to PDF for HR admin', async () => {
            ...
            mockAuditLogRepository.logDataExport.mockResolvedValue({});
            const result = await exportService.exportEmployeeRoster(mockEmployeeRosterReport, pdfOptions, mockHRAdminContext);
            expect(result.format).toBe('PDF');
            expect(result.recordCount).toBe(2);
            expect(mockPDFDoc.text).toHaveBeenCalledWith(expect.stringContaining('Employee Roster'), expect.any(Object));
        });
        it('should filter sensitive data for managers in CSV export', async () => {
            ...
            createObjectCsvWriter.mockReturnValue(mockCsvWriter);
            mockAuditLogRepository.logDataExport.mockResolvedValue({});
            await exportService.exportEmployeeRoster(mockEmployeeRosterReport, csvOptions, mockManagerContext);
        ...
        it('should apply field filtering when includeFields is specified', async () => {
            ...
            await exportService.exportEmployeeRoster(mockEmployeeRosterReport, filteredOptions, mockHRAdminContext);
            const writtenRecords = mockCsvWriter.writeRecords.mock.calls[0][0];
        ...
        it('should throw error for insufficient permissions', async () => {
            await expect(exportService.exportEmployeeRoster(mockEmployeeRosterReport, csvOptions, mockEmployeeContext)).rejects.toThrow(ValidationError);
            await expect(exportService.exportEmployeeRoster(mockEmployeeRosterReport, csvOptions, mockEmployeeContext)).rejects.toThrow('Insufficient permissions to export employee data');
        });
        it('should throw error for unsupported format', async () => {
            const invalidOptions = { format: 'XML' };
            await expect(exportService.exportEmployeeRoster(mockEmployeeRosterReport, invalidOptions, mockHRAdminContext)).rejects.toThrow(ValidationError);
            await expect(exportService.exportEmployeeRoster(mockEmployeeRosterReport, invalidOptions, mockHRAdminContext)).rejects.toThrow('Unsupported export format');
        });
    });
    describe('exportDepartmentBreakdown', () => {
        ...
        it('should export department breakdown to CSV', async () => {
            ...
            createObjectCsvWriter.mockReturnValue(mockCsvWriter);
            mockAuditLogRepository.logDataExport.mockResolvedValue({});
            const result = await exportService.exportDepartmentBreakdown(mockDepartmentBreakdownReport, csvOptions, mockHRAdminContext);
            expect(result.format).toBe('CSV');
        ...
        it('should throw error for insufficient permissions', async () => {
            await expect(exportService.exportDepartmentBreakdown(mockDepartmentBreakdownReport, csvOptions, mockEmployeeContext)).rejects.toThrow(ValidationError);
        });
    });
    describe('exportWorkforceAnalytics', () => {
        ...
        it('should export workforce analytics to CSV for HR admin', async () => {
            ...
            createObjectCsvWriter.mockReturnValue(mockCsvWriter);
            mockAuditLogRepository.logDataExport.mockResolvedValue({});
            const result = await exportService.exportWorkforceAnalytics(mockWorkforceAnalytics, csvOptions, mockHRAdminContext);
            expect(result.format).toBe('CSV');
            expect(result.recordCount).toBe(1);
            expect(mockCsvWriter.writeRecords).toHaveBeenCalledWith(expect.arrayContaining([
                expect.objectContaining({
        ...
        it('should throw error for non-HR admin users', async () => {
            await expect(exportService.exportWorkforceAnalytics(mockWorkforceAnalytics, csvOptions, mockManagerContext)).rejects.toThrow(ValidationError);
            await expect(exportService.exportWorkforceAnalytics(mockWorkforceAnalytics, csvOptions, mockManagerContext)).rejects.toThrow('Insufficient permissions to export workforce analytics');
        });
    });
    describe('prepareEmployeeRosterForCSV', () => {
        it('should include salary for HR admin', () => {
            const result = exportService.prepareEmployeeRosterForCSV([mockEmployee1], mockHRAdminContext, { format: 'CSV' });
        ...
        it('should exclude salary for managers', () => {
            const result = exportService.prepareEmployeeRosterForCSV([mockEmployee1], mockManagerContext, { format: 'CSV' });
    ...
    describe('applyFieldFiltering', () => {
        ...
        it('should include only specified fields when includeFields is provided', () => {
            ...
            const result = exportService.applyFieldFiltering(sampleData, options);
    ...
    describe('cleanupOldExports', () => {
        it('should delete files older than specified age', async () => {
            ...
            mockFs.readdirSync.mockReturnValue(['old-file.csv', 'new-file.csv']);
            mockFs.statSync
            ...
            expect(mockFs.unlinkSync).toHaveBeenCalledWith(expect.stringContaining('old-file.csv'));
            expect(mockFs.unlinkSync).not.toHaveBeenCalledWith(expect.stringContaining('new-file.csv'));
        });
        it('should not delete any files when all are recent', async () => {
            const recentDate = new Date(Date.now() - (1 * 60 * 60 * 1000)); // 1 hour ago
            mockFs.readdirSync.mockReturnValue(['recent-file.csv']);
            mockFs.statSync.mockReturnValue({ mtime: recentDate });
            const deletedCount = await exportService.cleanupOldExports(24);
