...
export class ExportService {
     ...
     */
    async exportEmployeeRoster(report, options, permissionContext) {
        // Check permissions
     ...
     */
    async exportDepartmentBreakdown(report, options, permissionContext) {
        // Check permissions
        if (permissionContext.role !== 'HR_ADMIN' && permissionContext.role !== 'MANAGER') {
     ...
     */
    async exportWorkforceAnalytics(analytics, options, permissionContext) {
    ...
    async exportToCSV(exportData, options) {
        ...
        switch (exportData.type) {
            case 'EMPLOYEE_ROSTER':
                const rosterData = exportData.data;
            ...
            case 'DEPARTMENT_BREAKDOWN':
                const deptData = exportData.data;
            ...
            case 'WORKFORCE_ANALYTICS':
                const analyticsData = exportData.data;
        ...
        // Log export activity
        await this.auditLogRepository.logDataExport(exportData.type, { format: 'CSV', recordCount, fileName }, exportData.permissionContext.userId, {
            action: 'csv_export',
    ...
    async exportToPDF(exportData, options) {
        ...
        // Log export activity
        await this.auditLogRepository.logDataExport(exportData.type, { format: 'PDF', recordCount, fileName }, exportData.permissionContext.userId, {
            action: 'pdf_export',
            filters: options.includeFields || options.excludeFields ? { includeFields: options.includeFields, excludeFields: options.excludeFields } : undefined
        });
        return {
     ...
     */
    prepareEmployeeRosterForCSV(employees, permissionContext, options) {
        return employees.map(employee => {
            ...
            if (permissionContext.role === 'HR_ADMIN') {
                if (employee.personalInfo.phone) {
                    baseData['Phone'] = employee.personalInfo.phone;
                }
                if (employee.personalInfo.dateOfBirth) {
                    baseData['Date of Birth'] = employee.personalInfo.dateOfBirth.toISOString().split('T')[0];
                }
                if (employee.jobInfo.salary) {
                    baseData['Salary'] = employee.jobInfo.salary;
                }
            }
     ...
     */
    prepareDepartmentBreakdownForCSV(departments) {
     ...
     */
    prepareWorkforceAnalyticsForCSV(analytics) {
        return [{
     ...
     */
    async addEmployeeRosterToPDF(doc, report, permissionContext, _options) {
     ...
     */
    async addDepartmentBreakdownToPDF(doc, report) {
     ...
     */
    async addWorkforceAnalyticsToPDF(doc, analytics) {
