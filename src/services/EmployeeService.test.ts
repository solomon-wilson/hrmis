import { EmployeeService, CreateEmployeeRequest, UpdateEmployeeRequest, PermissionContext } from './EmployeeService';
...
describe('EmployeeService', () => {
  ...
  beforeEach(() => {
    ...
    (employeeService as any).auditLogRepository = mockAuditLogRepository;
  });
  ...
  describe('createEmployee', () => {
    ...
    const hrAdminContext: PermissionContext = {
      userId: 'hr-admin-123',
      role: 'HR_ADMIN'
    };
    ...
    it('should create employee successfully with valid data and HR_ADMIN permissions', async () => {
      ...
      mockAuditLogRepository.logEmployeeCreate.mockResolvedValue({} as any);
    ...
    it('should throw ValidationError when user lacks HR_ADMIN permissions', async () => {
      // Act & Assert
      await expect(
        employeeService.createEmployee(validCreateRequest, managerContext)
    ...
    it('should throw ValidationError when email already exists', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.createEmployee(validCreateRequest, hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.createEmployee(validCreateRequest, hrAdminContext)
    ...
    it('should throw ValidationError when manager does not exist', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.createEmployee(requestWithManager, hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.createEmployee(requestWithManager, hrAdminContext)
      ).rejects.toThrow('Specified manager does not exist');
    });
    ...
    it('should throw ValidationError when manager is not active', async () => {
      ...
      await expect(
        employeeService.createEmployee(requestWithManager, hrAdminContext)
      ).rejects.toThrow('Specified manager is not active');
    });
  ...
  describe('updateEmployee', () => {
    ...
    it('should update employee successfully with valid data and permissions', async () => {
      ...
      mockAuditLogRepository.logEmployeeUpdate.mockResolvedValue({} as any);
    ...
    it('should throw ValidationError when employee not found', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.updateEmployee('non-existent', updateRequest, hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.updateEmployee('non-existent', updateRequest, hrAdminContext)
    ...
    it('should throw ValidationError when manager tries to update non-direct report', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.updateEmployee('emp-123', updateRequest, managerContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.updateEmployee('emp-123', updateRequest, managerContext)
      ).rejects.toThrow('Managers can only update their direct reports');
    });
    ...
    it('should throw ValidationError when employee tries to update other employee', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.updateEmployee('emp-123', updateRequest, employeeContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.updateEmployee('emp-123', updateRequest, employeeContext)
      ).rejects.toThrow('Employees can only update their own profile');
    });
    ...
    it('should throw ValidationError when email already exists for different employee', async () => {
      ...
      const emailUpdateRequest: UpdateEmployeeRequest = {
        personalInfo: {
          email: 'new.email@company.com'
        },
        updatedBy: TEST_UUID_1
      ...
      // Act & Assert
      await expect(
        employeeService.updateEmployee('emp-123', emailUpdateRequest, hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.updateEmployee('emp-123', emailUpdateRequest, hrAdminContext)
      ).rejects.toThrow('Employee with this email already exists');
    ...
    it('should prevent employee from being their own manager', async () => {
      // Arrange
      const managerUpdateRequest: UpdateEmployeeRequest = {
        jobInfo: {
      ...
      // Act & Assert
      await expect(
        employeeService.updateEmployee('emp-123', managerUpdateRequest, hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.updateEmployee('emp-123', managerUpdateRequest, hrAdminContext)
      ).rejects.toThrow('Employee cannot be their own manager');
    });
  });
  ...
  describe('getEmployee', () => {
    ...
    it('should throw ValidationError when manager tries to view non-direct report', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.getEmployee('emp-123', managerContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.getEmployee('emp-123', managerContext)
      ).rejects.toThrow('Insufficient permissions to view this employee');
    });
  ...
  describe('getDirectReports', () => {
    ...
    it('should throw ValidationError when non-manager tries to view other manager reports', async () => {
      // Arrange
      const otherManagerContext: PermissionContext = {
      ...
      // Act & Assert
      await expect(
        employeeService.getDirectReports('manager-123', otherManagerContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.getDirectReports('manager-123', otherManagerContext)
      ).rejects.toThrow('Insufficient permissions to view direct reports');
    });
  ...
  describe('updateEmployeeStatus', () => {
    ...
    it('should update employee status successfully with valid data and HR_ADMIN permissions', async () => {
      ...
      mockAuditLogRepository.logEmployeeStatusChange.mockResolvedValue({} as any);
    ...
    it('should throw ValidationError when user lacks HR_ADMIN permissions', async () => {
      ...
      ).rejects.toThrow('Insufficient permissions to change employee status');
    });
    ...
    it('should throw ValidationError when trying to change status of terminated employee', async () => {
      ...
      ).rejects.toThrow('Cannot change status of terminated employee');
    });
    ...
    it('should throw ValidationError when reason is missing', async () => {
      ...
      ).rejects.toThrow('Reason is required for status changes');
    });
    ...
    it('should throw ValidationError for invalid termination reason', async () => {
      ...
      ).rejects.toThrow('Invalid termination reason');
    });
    ...
    it('should throw ValidationError for invalid leave reason', async () => {
      ...
      ).rejects.toThrow('Invalid leave reason');
    });
    ...
    it('should throw ValidationError when termination date is in the future', async () => {
      ...
      ).rejects.toThrow('Termination effective date cannot be in the future');
    });
    ...
    it('should throw ValidationError when termination date is before start date', async () => {
      ...
      ).rejects.toThrow('Termination effective date cannot be before start date');
    });
    ...
    it('should throw ValidationError for retirement with insufficient service', async () => {
      ...
      ).rejects.toThrow('Retirement requires at least 1 year of service');
    });
    ...
    it('should throw ValidationError when leave date is too far in the past', async () => {
      ...
      ).rejects.toThrow('Leave effective date cannot be more than 30 days in the past');
    });
    ...
    it('should throw ValidationError when non-active employee tries to go on leave', async () => {
      ...
      ).rejects.toThrow('Only active employees can go on leave');
    });
  ...
  describe('getEmployeeStatusHistory', () => {
    ...
    it('should throw ValidationError when employee not found', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.getEmployeeStatusHistory('non-existent', hrAdminContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.getEmployeeStatusHistory('non-existent', hrAdminContext)
      ).rejects.toThrow('Employee not found');
    ...
    it('should throw ValidationError when manager tries to view non-direct report', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.getEmployeeStatusHistory(employee.id, managerContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.getEmployeeStatusHistory(employee.id, managerContext)
      ).rejects.toThrow('Managers can only view status history of their direct reports');
    });
    ...
    it('should throw ValidationError for insufficient permissions', async () => {
      ...
      // Act & Assert
      await expect(
        employeeService.getEmployeeStatusHistory(employee.id, viewerContext)
      ).rejects.toThrow(ValidationError);
      await expect(
        employeeService.getEmployeeStatusHistory(employee.id, viewerContext)
      ).rejects.toThrow('Insufficient permissions to view status history');
    });
  });
  ...
  describe('Organizational Hierarchy Management', () => {
    ...
    describe('updateManagerEmployeeRelationship', () => {
      it('should update manager relationship successfully with HR_ADMIN permissions', async () => {
        ...
        // Act
        const result = await employeeService.updateManagerEmployeeRelationship(
          employee.id,
          seniorManager.id,
          hrAdminContext
        );
      ...
      it('should throw ValidationError when user lacks HR_ADMIN permissions', async () => {
        // Arrange
        const nonHrContext: PermissionContext = {
          userId: 'manager-123',
          role: 'MANAGER'
        };
        ...
        // Act & Assert
        await expect(
          employeeService.updateManagerEmployeeRelationship(
            employee.id,
            seniorManager.id,
            nonHrContext
          )
        ...
        ).rejects.toThrow('Insufficient permissions to change reporting relationships');
      });
      ...
      it('should prevent circular reporting relationships', async () => {
        ...
        ).rejects.toThrow('Circular reporting relationship detected');
      });
    });
    ...
    describe('getEmployeeHierarchy', () => {
      ...
      it('should throw ValidationError when manager tries to view non-direct report hierarchy', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getEmployeeHierarchy(otherEmployee.id, managerContext)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getEmployeeHierarchy(otherEmployee.id, managerContext)
        ).rejects.toThrow('Managers can only view hierarchy of their direct reports');
      });
      ...
      it('should throw ValidationError when employee not found', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getEmployeeHierarchy('non-existent', hrAdminContext)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getEmployeeHierarchy('non-existent', hrAdminContext)
        ).rejects.toThrow('Employee not found');
    ...
    describe('getManagerOrganization', () => {
      ...
      it('should throw ValidationError when non-manager tries to view other manager organization', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getManagerOrganization(manager.id, otherManagerContext, false)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getManagerOrganization(manager.id, otherManagerContext, false)
        ).rejects.toThrow('Insufficient permissions to view organization');
      });
      ...
      it('should throw ValidationError when manager not found', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getManagerOrganization('non-existent', hrAdminContext, false)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getManagerOrganization('non-existent', hrAdminContext, false)
    ...
    describe('getOrganizationalChart', () => {
      ...
      it('should throw ValidationError when non-manager/HR tries to view chart', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getOrganizationalChart(manager.id, employeeContext, 2)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getOrganizationalChart(manager.id, employeeContext, 2)
        ).rejects.toThrow('Insufficient permissions to view organizational chart');
      });
      ...
      it('should throw ValidationError when manager tries to view other manager chart', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getOrganizationalChart(manager.id, otherManagerContext, 2)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getOrganizationalChart(manager.id, otherManagerContext, 2)
        ).rejects.toThrow('Managers can only view their own organizational chart');
      });
      ...
      it('should throw ValidationError when root manager not found', async () => {
        ...
        // Act & Assert
        await expect(
          employeeService.getOrganizationalChart('non-existent', hrAdminContext, 2)
        ).rejects.toThrow(ValidationError);
        await expect(
          employeeService.getOrganizationalChart('non-existent', hrAdminContext, 2)
        ).rejects.toThrow('Root manager not found');
      });
    ...
    describe('validateManagerRelationshipUpdate', () => {
      it('should return valid for legitimate manager relationship', async () => {
        ...
        // Act
        const result = await employeeService.validateManagerRelationshipUpdate(
          employee.id,
          manager.id,
          hrAdminContext
        );
