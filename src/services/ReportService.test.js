...
describe('ReportService', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        mockEmployeeRepository = new EmployeeRepository();
        mockAuditLogRepository = new AuditLogRepository();
        reportService = new ReportService();
        reportService.employeeRepository = mockEmployeeRepository;
        reportService.auditLogRepository = mockAuditLogRepository;
    });
    describe('generateEmployeeRosterReport', () => {
        it('should generate employee roster report for HR admin', async () => {
            ...
            mockAuditLogRepository.logReportGeneration.mockResolvedValue({});
            ...
            expect(mockAuditLogRepository.logReportGeneration).toHaveBeenCalledWith('EMPLOYEE_ROSTER', filters, 2, mockHRAdminContext.userId, { action: 'report_generated' }, expect.any(Object));
        });
        ...
        it('should throw error for insufficient permissions', async () => {
            const filters = {};
            await expect(reportService.generateEmployeeRosterReport(filters, mockEmployeeContext)).rejects.toThrow(ValidationError);
            await expect(reportService.generateEmployeeRosterReport(filters, mockEmployeeContext)).rejects.toThrow('Insufficient permissions to generate reports');
        });
    ...
    describe('generateDepartmentBreakdownReport', () => {
        it('should generate department breakdown report for HR admin', async () => {
            ...
            reportService.executeQuery = jest.fn().mockResolvedValue({
                rows: [
                    {
                        department_id: 'dept1',
        ...
        it('should throw error for insufficient permissions', async () => {
            const filters = {};
            await expect(reportService.generateDepartmentBreakdownReport(filters, mockEmployeeContext)).rejects.toThrow(ValidationError);
        });
    });
    describe('generateWorkforceAnalytics', () => {
        it('should generate comprehensive workforce analytics for HR admin', async () => {
            ...
            reportService.getDepartmentSummaries = jest.fn().mockResolvedValue([
                {
                    departmentId: 'dept1',
        ...
        it('should throw error for non-HR admin users', async () => {
            const filters = {};
            await expect(reportService.generateWorkforceAnalytics(filters, mockManagerContext)).rejects.toThrow(ValidationError);
            await expect(reportService.generateWorkforceAnalytics(filters, mockManagerContext)).rejects.toThrow('Insufficient permissions to generate workforce analytics');
        });
    ...
    describe('convertFiltersToSearchCriteria', () => {
        it('should convert report filters to search criteria', () => {
            ...
            const result = reportService.convertFiltersToSearchCriteria(filters);
    ...
    describe('filterEmployeeDataForReport', () => {
        it('should not filter data for HR admin', () => {
            ...
            const result = reportService.filterEmployeeDataForReport(employeeWithSensitiveData, mockHRAdminContext);
        ...
        it('should filter salary and SSN for managers', () => {
            ...
            const result = reportService.filterEmployeeDataForReport(employeeWithSensitiveData, mockManagerContext);
        ...
        it('should filter most sensitive data for other roles', () => {
            ...
            const result = reportService.filterEmployeeDataForReport(employeeWithSensitiveData, mockEmployeeContext);
            expect(result.jobInfo.salary).toBeUndefined();
            expect(result.personalInfo.socialSecurityNumber).toBeUndefined();
