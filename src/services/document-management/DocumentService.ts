...
export class DocumentService {
   ...
   */
  public static async uploadDocument(request: DocumentUploadRequest): Promise<StaffDocument> {
    return database.transaction(async (client: PoolClient) => {
      try {
        ...
        const documentData: StaffDocumentData = {
          employeeId: request.employeeId,
          category: request.category,
          title: request.title,
   ...
   */
  public static async getDocument(documentId: string, requestedBy: string): Promise<StaffDocument> {
   ...
   */
  public static async getDocumentDownloadUrl(documentId: string, requestedBy: string): Promise<string> {
  ...
  ): Promise<{ documents: StaffDocument[]; total: number }> {
    const client = await database.getClient();
   ...
   * Approve a document
   */
  public static async approveDocument(documentId: string, approvedBy: string): Promise<StaffDocument> {
    return database.transaction(async (client: PoolClient) => {
   ...
   * Reject a document
   */
  public static async rejectDocument(documentId: string, rejectedBy: string, reason?: string): Promise<StaffDocument> {
    return database.transaction(async (client: PoolClient) => {
   ...
   */
  public static async replaceDocument(
    documentId: string,
    newFile: Buffer,
    newFileName: string,
    newMimeType: string,
   ...
   * Delete a document
   */
  public static async deleteDocument(documentId: string, deletedBy: string, reason?: string): Promise<void> {
    return database.transaction(async (client: PoolClient) => {
  ...
  private static async checkDocumentPermission(
    documentId: string,
    userId: string,
    action: 'READ' | 'MODIFY' | 'APPROVE' | 'DELETE'
  ): Promise<boolean> {
   ...
   */
  public static async getExpiringDocuments(daysThreshold: number = 30): Promise<StaffDocument[]> {
    const client = await database.getClient();
