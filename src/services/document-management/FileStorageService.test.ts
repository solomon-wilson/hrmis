...
describe('FileStorageService', () => {
  ...
  const validUploadRequest = {
    ...
    category: 'PERSONAL_IDENTIFICATION' as const,
    metadata: { test: 'data' }
  };
  ...
  describe('uploadFile', () => {
    it('should successfully upload a valid file', async () => {
      ...
      expect(mockStorageClient.upload).toHaveBeenCalledWith(
        expect.stringMatching(/^employees\/123e4567-e89b-12d3-a456-426614174000\/personal_identification\/\d{4}-\d{2}-\d{2}\//),
        mockFile,
        {
  ...
  describe('getDownloadUrl', () => {
    it('should generate signed URL for document download', async () => {
      ...
      const url = await FileStorageService.getDownloadUrl(
    ...
    it('should handle signed URL generation errors', async () => {
      ...
      await expect(FileStorageService.getDownloadUrl(
        'employees/123/personal_identification/document.pdf',
        'PERSONAL_IDENTIFICATION'
  ...
  describe('deleteFile', () => {
    ...
    it('should handle deletion errors', async () => {
      ...
      await expect(FileStorageService.deleteFile(
        'employees/123/personal_identification/document.pdf',
        'PERSONAL_IDENTIFICATION'
      )).rejects.toThrow(ValidationError);
    });
  ...
  describe('listFiles', () => {
    it('should list files for an employee', async () => {
      ...
      expect(mockStorageClient.list).toHaveBeenCalledWith(
    ...
    it('should list files for specific category', async () => {
      ...
      expect(mockStorageClient.list).toHaveBeenCalledWith(
        'employees/123e4567-e89b-12d3-a456-426614174000/passport_photo/',
        {
          limit: 100,
          sortBy: { column: 'created_at', order: 'desc' }
        }
  ...
  describe('passport photo validation', () => {
    it('should validate passport photos are images', async () => {
      const request = {
        ...validUploadRequest,
        category: 'PASSPORT_PHOTO' as const,
        mimeType: 'application/pdf',
        fileName: 'photo.pdf'
      };
