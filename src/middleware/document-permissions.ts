...
/**
 * Document permission middleware for role-based access control
 */
...
/**
 * Enhanced request interface with document permission context
 */
 ...
 */
export const documentPermissionMiddleware = async (
  req: DocumentRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const userContext: DocumentPermissionContext = {
    ...
    const permissions = await calculateDocumentPermissions(
      userContext,
      targetEmployeeId,
      documentId,
      req.method,
 ...
 */
export const documentUploadPermissionMiddleware = async (
  req: DocumentRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const canUpload = await checkDocumentUploadPermission(
      userContext,
      targetEmployeeId,
      category
    );
 ...
 */
export const documentApprovalPermissionMiddleware = async (
  req: DocumentRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
 ...
 */
export const leavePlanPermissionMiddleware = async (
  req: DocumentRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const permissions = await calculateLeavePlanPermissions(
      userContext,
      targetEmployeeId,
      planId,
      req.method
 ...
 */
async function calculateDocumentPermissions(
 ...
 */
async function checkDocumentUploadPermission(
  userContext: DocumentPermissionContext,
  targetEmployeeId: string,
  ...
  if (targetEmployeeId === userContext.employeeId || targetEmployeeId === userContext.userId) {
    ...
    const restrictedCategories: DocumentCategory[] = ['PERFORMANCE_REVIEW'];
  ...
  if (userContext.roles.includes('MANAGER') &&
      userContext.managedEmployeeIds?.includes(targetEmployeeId)) {
    const managerAllowedCategories: DocumentCategory[] = [
      'PERFORMANCE_REVIEW',
      'TRAINING_RECORD'
    ];
 ...
 */
async function calculateLeavePlanPermissions(
  userContext: DocumentPermissionContext,
  targetEmployeeId?: string,
  planId?: string,
  method?: string
 ...
 */
export const canAccessEmployeeDocuments = (
  userContext: DocumentPermissionContext,
  targetEmployeeId: string
): boolean => {
 ...
 */
export const documentRateLimitMiddleware = (
  windowMs: number = 15 * 60 * 1000, // 15 minutes
  maxRequests: number = 50 // 50 requests per window
) => {
