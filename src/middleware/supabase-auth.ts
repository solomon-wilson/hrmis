...
import { permissionManager, PermissionContext, Resource, Action } from '../services/PermissionManager';
 ...
 */
export const authenticateWithSupabase = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
 ...
 */
export const optionalAuthenticateWithSupabase = async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
  try {
    const authHeader = req.headers.authorization;
    const token = supabaseAuthService.extractTokenFromHeader(authHeader);
...
export const requireRole = (requiredRole: string) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const hasRole = permissionManager.hasRole(req.permissionContext.roles, requiredRole as any);
 ...
 */
export const canAccessEmployee = (req: Request, res: Response, next: NextFunction): void => {
  if (!req.user || !req.permissionContext) {
    res.status(401).json({
      error: {
        code: 'AUTHENTICATION_REQUIRED',
 ...
 */
export const canModifyEmployee = (req: Request, res: Response, next: NextFunction): void => {
  if (!req.user || !req.permissionContext) {
    res.status(401).json({
      error: {
        code: 'AUTHENTICATION_REQUIRED',
 ...
 */
export const filterEmployeeFields = (req: Request, res: Response, next: NextFunction): void => {
  ...
  res.json = function(data: any) {
    if (data && typeof data === 'object') {
      ...
      if (data.data && Array.isArray(data.data)) {
        const filteredData = data.data.map((employee: any) =>
          permissionManager.filterSensitiveFields(
            employee,
            req.permissionContext!,
            'employee'
          )
 ...
 */
export const addManagerContext = async (req: Request, _res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (isManager && req.employee) {
      try {
        ...
        const directReports = await employeeRepository.getDirectReports(
          req.employee.getId(),
          req.accessToken
        );
 ...
 */
export const requireEmployeeRecord = (req: Request, res: Response, next: NextFunction): void => {
  if (!req.employee) {
    res.status(403).json({
      error: {
