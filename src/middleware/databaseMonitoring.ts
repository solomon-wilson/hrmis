...
export class MonitoredDatabaseClient {
  ...
  constructor(client: PoolClient, correlationId?: string) {
    this.client = client;
   ...
   */
  private getQueryType(query: string): 'SELECT' | 'INSERT' | 'UPDATE' | 'DELETE' | 'OTHER' {
...
export class MonitoredDatabasePool {
   ...
   */
  async getClient(correlationId?: string): Promise<MonitoredDatabaseClient> {
   ...
   */
  async query(text: string, params?: any[], correlationId?: string): Promise<any> {
...
export function withDatabaseCorrelationId<T extends any[], R>(
  fn: (...args: T) => Promise<R>
) {
  return async function(this: any, ...args: T): Promise<R> {
    // Try to get correlation ID from various sources
    const correlationId = 
      (this as any).correlationId || 
      (args[0] as any)?.correlationId ||
