import { AuditLogRepository, CreateAuditLogInput } from '../audit';
...
describe('AuditLogRepository', () => {
  ...
  describe('findAll', () => {
    ...
    it('should sort by performed_at in descending order by default', async () => {
      ...
      for (let i = 0; i < result.data.length - 1; i++) {
        expect(result.data[i].performedAt.getTime()).toBeGreaterThanOrEqual(
          result.data[i + 1].performedAt.getTime()
        );
      }
    });
  });
  ...
  describe('update and delete', () => {
    it('should throw error when trying to update audit log', async () => {
      ...
      await expect(repository.update(created.id, {})).rejects.toThrow(
  ...
  describe('Employee-specific logging methods', () => {
    describe('logEmployeeCreate', () => {
      it('should log employee creation', async () => {
        ...
        const result = await repository.logEmployeeCreate(
          'emp-123',
          employeeData,
          testUser.id,
          { source: 'HR_SYSTEM' }
    ...
    describe('logEmployeeUpdate', () => {
      it('should log employee update with before and after data', async () => {
        ...
        const result = await repository.logEmployeeUpdate(
          'emp-123',
          beforeData,
          afterData,
          testUser.id
    ...
    describe('logEmployeeStatusChange', () => {
      it('should log employee status change', async () => {
        const result = await repository.logEmployeeStatusChange(
          'emp-123',
          'ACTIVE',
          'TERMINATED',
          'End of contract',
    ...
    describe('logEmployeeDelete', () => {
      it('should log employee deletion', async () => {
        ...
        const result = await repository.logEmployeeDelete(
          'emp-123',
          employeeData,
          testUser.id
        );
  ...
  describe('User activity logging methods', () => {
    describe('logUserLogin', () => {
      it('should log user login', async () => {
        const result = await repository.logUserLogin(
          testUser.id,
          '192.168.1.1',
          'Mozilla/5.0',
          'session-123'
    ...
    describe('logUserLogout', () => {
      it('should log user logout', async () => {
        const result = await repository.logUserLogout(
          testUser.id,
          'session-123'
        );
  ...
  describe('logDataExport', () => {
    it('should log data export', async () => {
      ...
      const result = await repository.logDataExport(
        'EMPLOYEE_ROSTER',
        exportedData,
        testUser.id,
        { format: 'CSV' }
