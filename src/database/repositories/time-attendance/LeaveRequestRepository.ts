...
import { LeaveRequest, LeaveRequestData } from '../../../models/time-attendance/LeaveRequest';
...
export interface LeaveRequestFilters extends FilterOptions {
  employeeId?: string;
  leaveTypeId?: string;
  status?: 'PENDING' | 'APPROVED' | 'REJECTED' | 'CANCELLED';
  dateRange?: {
  ...
  approvedBy?: string;
  managerId?: string; // For manager dashboard filtering
...
export interface LeaveConflict {
  ...
  conflictType: 'OVERLAP' | 'SAME_DATES' | 'ADJACENT';
...
export class LeaveRequestRepository extends SupabaseRepository<LeaveRequest, LeaveRequestCreateInput, LeaveRequestUpdateInput> {
  constructor() {
   ...
   */
  async create(data: LeaveRequestCreateInput, userContext?: string): Promise<LeaveRequest> {
    const client = this.getClient(userContext);

    // Validate for conflicts before creating
    const conflicts = await this.checkLeaveConflicts(
      data.employeeId,
      data.startDate,
      data.endDate,
      userContext
    ...
    const leaveRequest = await this.executeQuery<any>(
      client.from(this.tableName).insert(leaveRequestData).select().single(),
   ...
   */
  async findById(id: string, userContext?: string): Promise<LeaveRequest | null> {
    const client = this.getClient(userContext);
   ...
   */
  async update(id: string, data: LeaveRequestUpdateInput, userContext?: string): Promise<LeaveRequest | null> {
    const client = this.getClient(userContext);
    ...
    const updatedRequest = await this.executeQuery<any>(
      client
        .from(this.tableName)
        .update(updateData)
        .eq('id', id)
  ...
  }, userContext?: string): Promise<PaginatedResult<LeaveRequest>> {
   ...
   */
  async processApproval(approval: LeaveRequestApproval, userContext?: string): Promise<LeaveRequest | null> {
  ...
  async checkLeaveConflicts(
    ...
    const conflictingRequests = await this.executeQuery<any[]>(query, 'check leave conflicts');
    ...
    for (const conflicting of conflictingRequests) {
      ...
      let conflictType: 'OVERLAP' | 'SAME_DATES' | 'ADJACENT';
      ...
      if (startDate.getTime() === conflictStart.getTime() && endDate.getTime() === conflictEnd.getTime()) {
        ...
        (startDate <= conflictEnd && endDate >= conflictStart)
      ) {
   ...
   */
  async findPendingForManager(managerId: string, userContext?: string): Promise<LeaveRequest[]> {
   ...
   */
  async cancelRequest(requestId: string, reason?: string, userContext?: string): Promise<LeaveRequest | null> {
   ...
   */
  async findRequiringApproval(userContext?: string): Promise<LeaveRequest[]> {
    const client = this.getClient(userContext);
