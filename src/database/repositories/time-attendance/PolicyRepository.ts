...
import { LeavePolicy, LeavePolicyData, OvertimePolicy, OvertimePolicyData } from '../../../models/time-attendance/Policy';

export interface PolicyCreateInput {
  name: string;
  description: string;
  leaveTypeId?: string; // For leave policies
  isActive: boolean;
  applicableGroups: string[];
  eligibilityRules: any[];
  accrualRules?: any[]; // For leave policies
  usageRules?: any[]; // For leave policies
  overtimeRules?: any; // For overtime policies
  effectiveDate: Date;
  expiryDate?: Date;
}
...
export interface PolicyFilters extends FilterOptions {
  policyType?: 'LEAVE' | 'OVERTIME';
  leaveTypeId?: string;
  isActive?: boolean;
  applicableGroup?: string;
  effectiveOn?: Date; // Policies effective on this date
  expiredBefore?: Date;
}
...
export interface PolicyConflict {
  policy1Id: string;
  policy2Id: string;
  conflictType: 'OVERLAP' | 'DUPLICATE_GROUPS' | 'SAME_LEAVE_TYPE';
  description: string;
}
...
export class PolicyRepository extends SupabaseRepository<LeavePolicy | OvertimePolicy, PolicyCreateInput, PolicyUpdateInput> {
  constructor() {
    super('policies');
  }
   ...
   * Create a new policy
   */
  async create(data: PolicyCreateInput, userContext?: string): Promise<LeavePolicy | OvertimePolicy> {
    ...
    const policy = await this.executeQuery<any>(
      client.from(this.tableName).insert(policyData).select().single(),
      'create policy'
    );
   ...
   * Find policy by ID
   */
  async findById(id: string, userContext?: string): Promise<LeavePolicy | OvertimePolicy | null> {
    const client = this.getClient(userContext);
   ...
   * Update policy
   */
  async update(id: string, data: PolicyUpdateInput, userContext?: string): Promise<LeavePolicy | OvertimePolicy | null> {
    const client = this.getClient(userContext);
    ...
    if (data.applicableGroups !== undefined) updateData.applicable_groups = JSON.stringify(data.applicableGroups);
    if (data.eligibilityRules !== undefined) updateData.eligibility_rules = JSON.stringify(data.eligibilityRules);
    if (data.accrualRules !== undefined) updateData.accrual_rules = JSON.stringify(data.accrualRules);
    if (data.usageRules !== undefined) updateData.usage_rules = JSON.stringify(data.usageRules);
    if (data.overtimeRules !== undefined) updateData.overtime_rules = JSON.stringify(data.overtimeRules);
    if (data.effectiveDate !== undefined) updateData.effective_date = data.effectiveDate.toISOString().split('T')[0];
    if (data.expiryDate !== undefined) updateData.expiry_date = data.expiryDate?.toISOString().split('T')[0] || null;

    const updatedPolicy = await this.executeQuery<any>(
      client
        .from(this.tableName)
        .update(updateData)
        .eq('id', id)
  ...
  }, userContext?: string): Promise<PaginatedResult<LeavePolicy | OvertimePolicy>> {
   ...
   */
  async findActiveLeavePolicies(userContext?: string): Promise<LeavePolicy[]> {
   ...
   */
  async findActiveOvertimePolicies(userContext?: string): Promise<OvertimePolicy[]> {
    const result = await this.findAll({
      filters: {
        policyType: 'OVERTIME',
        isActive: true,
    ...
    return result.data as OvertimePolicy[];
  }

  /**
   * Find policies by leave type
   */
  async findByLeaveType(leaveTypeId: string, userContext?: string): Promise<LeavePolicy[]> {
    const result = await this.findAll({
      filters: {
        leaveTypeId,
        isActive: true
   ...
   */
  async findApplicableToGroup(groupName: string, userContext?: string): Promise<(LeavePolicy | OvertimePolicy)[]> {
    const result = await this.findAll({
      filters: {
   ...
   */
  async checkPolicyConflicts(
    policyData: PolicyCreateInput,
    ...
    if (policyData.leaveTypeId) {
      ...
      for (const existing of existingPolicies) {
        if (excludePolicyId && existing.id === excludePolicyId) continue;

        // Check for overlapping groups
        const hasOverlappingGroups = policyData.applicableGroups.some(group =>
          existing.applicableGroups.includes(group)
        );
   ...
   * Activate policy
   */
  async activatePolicy(policyId: string, userContext?: string): Promise<LeavePolicy | OvertimePolicy | null> {
   ...
   * Deactivate policy
   */
  async deactivatePolicy(policyId: string, userContext?: string): Promise<LeavePolicy | OvertimePolicy | null> {
    return this.update(policyId, {
      isActive: false,
      expiryDate: new Date()
    }, userContext);
   ...
   */
  async getPolicyApplicationResult(
    policyId: string,
    userContext?: string
  ...
  async findExpiringPolicies(
    daysAhead: number = 30,
    userContext?: string
  ): Promise<(LeavePolicy | OvertimePolicy)[]> {
  ...
  async clonePolicy(
    policyId: string,
    modifications: Partial<PolicyCreateInput>,
    userContext?: string
  ): Promise<LeavePolicy | OvertimePolicy> {
    ...
    const cloneData: PolicyCreateInput = {
    ...
    if (originalPolicy instanceof LeavePolicy) {
      ...
      cloneData.overtimeRules = modifications.overtimeRules || (originalPolicy as OvertimePolicy).overtimeRules;
    }
   ...
   */
  private mapToPolicy(row: any): LeavePolicy | OvertimePolicy {
    if (row.policy_type === 'LEAVE') {
      const leavePolicyData: LeavePolicyData = {
        id: row.id,
        name: row.name,
      ...
      const overtimePolicyData: OvertimePolicyData = {
        id: row.id,
        name: row.name,
        description: row.description,
        isActive: row.is_active,
