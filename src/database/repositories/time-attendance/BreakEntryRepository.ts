...
import { BreakEntry, BreakEntryData } from '../../../models/time-attendance/TimeEntry';

export interface BreakEntryCreateInput {
  timeEntryId: string;
...
export interface BreakEntryUpdateInput {
  endTime?: Date;
  duration?: number;
  paid?: boolean;
}

export interface BreakEntryFilters extends FilterOptions {
  timeEntryId?: string;
  employeeId?: string;
  breakType?: 'LUNCH' | 'SHORT_BREAK' | 'PERSONAL';
  dateRange?: {
...
export class BreakEntryRepository extends SupabaseRepository<BreakEntry, BreakEntryCreateInput, BreakEntryUpdateInput> {
  constructor() {
    super('break_entries');
  }
   ...
   */
  async create(data: BreakEntryCreateInput, userContext?: string): Promise<BreakEntry> {
    ...
    const breakEntry = await this.executeQuery<any>(
      client.from(this.tableName).insert(breakEntryData).select().single(),
      'create break entry'
    );
   ...
   */
  async findById(id: string, userContext?: string): Promise<BreakEntry | null> {
    const client = this.getClient(userContext);
   ...
   * Update break entry
   */
  async update(id: string, data: BreakEntryUpdateInput, userContext?: string): Promise<BreakEntry | null> {
    const client = this.getClient(userContext);
  ...
  async findAll(options?: {
    ...
    sort?: SortOptions;
    filters?: BreakEntryFilters;
  }, userContext?: string): Promise<PaginatedResult<BreakEntry>> {
   ...
   */
  async findByTimeEntry(timeEntryId: string, userContext?: string): Promise<BreakEntry[]> {
   ...
   */
  async findActiveBreak(employeeId: string, userContext?: string): Promise<BreakEntry | null> {
    const client = this.getClient(userContext);
   ...
   */
  private mapToBreakEntry(row: any): BreakEntry {
    const breakEntryData: BreakEntryData = {
      id: row.id,
