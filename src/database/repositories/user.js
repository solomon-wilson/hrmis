...
export class UserRepository extends BaseRepository {
     ...
     */
    async findByUsername(username, client) {
        const query = `
      SELECT id, username, email, roles, employee_id, is_active, created_at, updated_at
      FROM users 
      WHERE username = $1
    ...
    `;
        try {
            ...
            const paginationMeta = this.calculatePaginationMeta(total, pagination?.page || 1, limit);
            return {
                data: users,
     ...
     */
    async emailExists(email, excludeId, client) {
     ...
     */
    async usernameExists(username, excludeId, client) {
     ...
     */
    async incrementTokenVersion(id, client) {
        const query = `
      UPDATE users 
