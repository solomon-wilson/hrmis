...
import { AnnualLeavePlan, AnnualLeaveStatus, PlannedLeave } from '../../models/document-management';
import { ValidationError } from '../../utils/validation';

export interface LeavePlanSearchCriteria {
  employeeId?: string;
  year?: number;
...
export interface LeavePlanListOptions {
  page?: number;
  limit?: number;
  sortBy?: 'created_at' | 'year' | 'submitted_at' | 'status';
  sortOrder?: 'asc' | 'desc';
}
...
export class AnnualLeavePlanRepository {
   ...
   */
  async create(plan: Omit<AnnualLeavePlan, 'id' | 'createdAt' | 'updatedAt'>): Promise<AnnualLeavePlan> {
    try {
   ...
   */
  async findById(id: string): Promise<AnnualLeavePlan | null> {
    try {
      const { data, error } = await this.client
        .from('annual_leave_plans')
        .select('*')
   ...
   */
  async findByEmployeeAndYear(employeeId: string, year: number): Promise<AnnualLeavePlan | null> {
    try {
      const { data, error } = await this.client
        .from('annual_leave_plans')
        .select('*')
   ...
   */
  async findByEmployeeId(employeeId: string, options: LeavePlanListOptions = {}): Promise<{
    plans: AnnualLeavePlan[];
    total: number;
    page: number;
    totalPages: number;
  }> {
   ...
   */
  async search(
    criteria: LeavePlanSearchCriteria,
    options: LeavePlanListOptions = {}
  ): Promise<{
    plans: AnnualLeavePlan[];
    total: number;
    page: number;
  ...
  async update(
    id: string,
    updates: Partial<AnnualLeavePlan>
  ): Promise<AnnualLeavePlan> {
    try {
      ...
      if (updates.totalEntitlement !== undefined) updateData.total_entitlement = updates.totalEntitlement;
      if (updates.carriedOver !== undefined) updateData.carried_over = updates.carriedOver;
      if (updates.plannedLeaves !== undefined) updateData.planned_leaves = updates.plannedLeaves;
      ...
      if (updates.managerApprovedBy !== undefined) updateData.manager_approved_by = updates.managerApprovedBy;
      if (updates.hrApprovedAt !== undefined) {
        updateData.hr_approved_at = updates.hrApprovedAt ? updates.hrApprovedAt.toISOString() : null;
      }
      if (updates.hrApprovedBy !== undefined) updateData.hr_approved_by = updates.hrApprovedBy;
   ...
   */
  async getPendingManagerApprovals(managerId: string): Promise<AnnualLeavePlan[]> {
    try {
   ...
   */
  async getPendingHRApprovals(): Promise<AnnualLeavePlan[]> {
    try {
      const { data, error } = await this.client
        .from('annual_leave_plans')
        .select('*')
  ...
  async checkConflicts(
    ...
    conflicts: Array<{
      existingPlan: AnnualLeavePlan;
      conflictingLeave: PlannedLeave;
  ...
  async getStatistics(year?: number): Promise<{
    totalPlans: number;
    byStatus: Record<AnnualLeaveStatus, number>;
    ...
    try {
      ...
      const stats = {
        ...
        byStatus: {
          ...
          'HR_APPROVED': 0,
          'REJECTED': 0
        } as Record<AnnualLeaveStatus, number>,
      ...
      data.forEach(planData => {
        // Count by status
        stats.byStatus[planData.status as AnnualLeaveStatus]++;

        // Sum entitlements
        totalEntitlement += parseFloat(planData.total_entitlement);

        // Sum planned days
        const plannedLeaves = planData.planned_leaves as PlannedLeave[];
   ...
   */
  private datesOverlap(leave1: PlannedLeave, leave2: PlannedLeave): boolean {
   ...
   */
  private mapToAnnualLeavePlan(data: any): AnnualLeavePlan {
