...
import { StaffDocument, DocumentCategory, DocumentStatus } from '../../models/document-management';
...
export interface DocumentListOptions {
  page?: number;
  limit?: number;
  sortBy?: 'created_at' | 'title' | 'expires_at' | 'status';
  sortOrder?: 'asc' | 'desc';
}
...
export class StaffDocumentRepository {
   ...
   */
  async create(document: Omit<StaffDocument, 'id' | 'createdAt' | 'updatedAt'>): Promise<StaffDocument> {
    try {
   ...
   */
  async findByEmployeeId(employeeId: string, options: DocumentListOptions = {}): Promise<{
   ...
   * Delete document
   */
  async delete(id: string, performedBy: string, reason?: string): Promise<boolean> {
    try {
   ...
   */
  async getExpiringDocuments(days: number = 30): Promise<StaffDocument[]> {
    try {
   ...
   */
  private mapToStaffDocument(data: any): StaffDocument {
