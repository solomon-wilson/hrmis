 ...
 */
router.get('/metrics', authenticate, authorize(['HR_ADMIN']), validateQuery(timeRangeSchema), MonitoringController.getMetrics);
/**
 * @route GET /api/monitoring/errors
 * @desc Get error tracking information
 * @access HR_ADMIN only
 */
router.get('/errors', authenticate, authorize(['HR_ADMIN']), validateQuery(timeRangeSchema), MonitoringController.getErrors);
/**
 * @route GET /api/monitoring/performance
 * @desc Get performance statistics
 * @access HR_ADMIN only
 */
router.get('/performance', authenticate, authorize(['HR_ADMIN']), validateQuery(timeRangeSchema), MonitoringController.getPerformance);
/**
 * @route GET /api/monitoring/health
 * @desc Get system health status
 * @access HR_ADMIN, MANAGER (limited access)
 */
router.get('/health', authenticate, authorize(['HR_ADMIN', 'MANAGER']), MonitoringController.getHealth);
/**
 * @route GET /api/monitoring/prometheus
 * @desc Export metrics in Prometheus format
 * @access HR_ADMIN only
 */
router.get('/prometheus', authenticate, authorize(['HR_ADMIN']), validateQuery(timeRangeSchema), MonitoringController.getPrometheusMetrics);
