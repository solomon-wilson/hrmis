import { AnnualLeavePlan, AnnualLeavePlanData, PlannedLeave } from './AnnualLeavePlan';
import { ValidationError } from '../../utils/validation';

describe('AnnualLeavePlan', () => {
  const currentYear = new Date().getFullYear();

  const validPlannedLeave: PlannedLeave = {
  ...
  const validLeavePlanData: AnnualLeavePlanData = {
  ...
  describe('business rules validation', () => {
    it('should reject plans where total planned exceeds entitlement', () => {
      const excessiveLeave: PlannedLeave = {
    ...
    it('should reject leave with end date before start date', () => {
      const invalidLeave: PlannedLeave = {
  ...
  describe('utility methods', () => {
    ...
    it('should filter leaves by type', () => {
      const personalLeave: PlannedLeave = {
        startDate: new Date(currentYear, 7, 1),
        endDate: new Date(currentYear, 7, 1),
        days: 1,
        type: 'PERSONAL'
    ...
    it('should check modification capability', () => {
      ...
      const rejectedData = { ...validLeavePlanData, status: 'REJECTED' as const, rejectionReason: 'Test' };
    ...
    it('should check approval status', () => {
      ...
      const submittedData = { ...validLeavePlanData, status: 'SUBMITTED' as const, submittedAt: new Date() };
