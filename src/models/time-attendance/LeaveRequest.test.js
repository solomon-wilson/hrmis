import { LeaveRequest, LeaveType } from './LeaveRequest';
...
describe('LeaveRequest', () => {
    ...
    describe('constructor', () => {
        ...
        it('should throw ValidationError for approved request without reviewer', () => {
            const data = { ...validRequestData, status: 'APPROVED' };
            expect(() => new LeaveRequest(data)).toThrow(ValidationError);
        });
    ...
    describe('createNew', () => {
        it('should create new leave request with pending status', () => {
            ...
            const request = LeaveRequest.createNew(employeeId, leaveTypeId, new Date('2025-12-01'), new Date('2025-12-05'), 5, 40, 'Vacation');
            expect(request.status).toBe('PENDING');
