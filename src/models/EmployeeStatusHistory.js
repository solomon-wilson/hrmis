...
import { validateAndThrow, ValidationError, optionalStringSchema, dateSchema, employeeStatusTypeSchema, uuidSchema } from '../utils/validation';
export class EmployeeStatusHistory {
    ...
    static createInitialStatus(employeeId, status, effectiveDate, changedBy, reason, notes) {
        const data = {
            employeeId,
            newStatus: status,
            effectiveDate,
    ...
    static createStatusChange(employeeId, previousStatus, newStatus, effectiveDate, changedBy, reason, notes) {
        const data = {
            employeeId,
            previousStatus,
            newStatus,
    ...
    static createTermination(employeeId, previousStatus, reason, effectiveDate, changedBy, notes) {
        return EmployeeStatusHistory.createStatusChange(employeeId, previousStatus, 'TERMINATED', effectiveDate, changedBy, reason, notes);
    }
    static createLeave(employeeId, previousStatus, reason, effectiveDate, changedBy, notes) {
        return EmployeeStatusHistory.createStatusChange(employeeId, previousStatus, 'ON_LEAVE', effectiveDate, changedBy, reason, notes);
    }
}
