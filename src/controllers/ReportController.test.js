...
describe('ReportController', () => {
    ...
    beforeEach(() => {
        ...
        reportController.reportService = mockReportService;
        app = express();
        app.use(express.json());
        app.use(authenticate);
        // Setup routes
        app.get('/reports/employees', (req, res) => reportController.generateEmployeeRoster(req, res));
        app.get('/reports/departments', (req, res) => reportController.generateDepartmentBreakdown(req, res));
        app.get('/reports/analytics', (req, res) => reportController.generateWorkforceAnalytics(req, res));
        app.post('/reports/employees/export', (req, res) => reportController.exportEmployeeRoster(req, res));
        app.post('/reports/departments/export', (req, res) => reportController.exportDepartmentBreakdown(req, res));
        app.post('/reports/analytics/export', (req, res) => reportController.exportWorkforceAnalytics(req, res));
    ...
    describe('GET /reports/employees', () => {
        it('should generate employee roster report successfully', async () => {
            ...
            expect(mockReportService.generateEmployeeRosterReport).toHaveBeenCalledWith({ status: 'ACTIVE', department: 'Engineering' }, mockPermissionContext);
        });
        it('should handle validation errors', async () => {
            mockReportService.generateEmployeeRosterReport.mockRejectedValue(new ValidationError('Invalid filter', []));
            const response = await request(app)
                .get('/reports/employees');
        ...
        it('should handle internal server errors', async () => {
            mockReportService.generateEmployeeRosterReport.mockRejectedValue(new Error('Database connection failed'));
            const response = await request(app)
                .get('/reports/employees');
    ...
    describe('POST /reports/employees/export', () => {
        it('should export employee roster to CSV successfully', async () => {
            ...
            expect(mockReportService.exportEmployeeRoster).toHaveBeenCalledWith({ status: 'ACTIVE' }, {
                format: 'CSV',
                includeFields: ['Employee ID', 'First Name', 'Last Name']
            }, mockPermissionContext);
        });
        it('should export employee roster to PDF successfully', async () => {
            const pdfExportResult = { ...mockExportResult, format: 'PDF', fileName: 'test-export.pdf' };
    ...
    describe('parseReportFilters', () => {
        it('should parse all filter types correctly', () => {
            ...
            const filters = reportController.parseReportFilters(query);
        ...
        it('should handle empty query parameters', () => {
            const filters = reportController.parseReportFilters({});
            expect(filters).toEqual({});
        });
    });
    describe('parseExportOptions', () => {
        it('should parse export options correctly', () => {
            ...
            const options = reportController.parseExportOptions(body);
        ...
        it('should default to CSV format', () => {
            const options = reportController.parseExportOptions({});
            expect(options.format).toBe('CSV');
        });
        it('should throw error for invalid format', () => {
            expect(() => {
                reportController.parseExportOptions({ format: 'INVALID' });
