...
import { AnnualLeavePlanRepository, LeavePlanSearchCriteria, LeavePlanListOptions } from '../database/repositories/AnnualLeavePlanRepository';
...
export class AnnualLeavePlanController {
  private leavePlanRepository: AnnualLeavePlanRepository;
   ...
   */
  public createLeavePlan = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let validatedPlannedLeaves: PlannedLeave[] = [];
      if (plannedLeaves && Array.isArray(plannedLeaves)) {
        try {
          validatedPlannedLeaves = plannedLeaves.map((leave: any) => {
          ...
          // Check for conflicts
          const conflictCheck = await this.leavePlanRepository.checkConflicts(
            employeeId,
            validatedPlannedLeaves
          );
   ...
   */
  public getLeavePlan = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
   ...
   * GET /api/leave-plans
   */
  public listLeavePlans = async (req: Request, res: Response): Promise<void> => {
    try {
      const currentUser = req.user!;
      ...
      const criteria: LeavePlanSearchCriteria = {};
      ...
      if (status) criteria.status = status as AnnualLeaveStatus;

      const options: LeavePlanListOptions = {
   ...
   */
  public updateLeavePlan = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      ...
      if (plannedLeaves && Array.isArray(plannedLeaves)) {
        try {
          ...
          const conflictCheck = await this.leavePlanRepository.checkConflicts(
            leavePlan.employeeId,
            updates.plannedLeaves,
            id
          );
   ...
   */
  public submitLeavePlan = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
   ...
   */
  public managerApproval = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
   ...
   */
  public hrApproval = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { reason } = req.body;
   ...
   */
  public deleteLeavePlan = async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
   ...
   */
  public getPendingManagerApprovals = async (req: Request, res: Response): Promise<void> => {
    try {
      const currentUser = req.user!;
   ...
   */
  public getPendingHRApprovals = async (req: Request, res: Response): Promise<void> => {
    try {
      const currentUser = req.user!;
  ...
  public getStatistics = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const stats = await this.leavePlanRepository.getStatistics(
