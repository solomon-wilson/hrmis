...
const MockedEmployeeService = EmployeeService;
...
describe('ManagerController Integration Tests', () => {
    ...
    describe('GET /api/managers/:id/reports', () => {
        it('should return direct reports for a manager', async () => {
            ...
            expect(mockEmployeeService.getDirectReports).toHaveBeenCalledWith(mockManager.id, expect.objectContaining({
                userId: mockUser.id,
                role: 'MANAGER'
            }));
        });
        ...
        it('should return 403 when user lacks permission to view reports', async () => {
            // Arrange
            mockEmployeeService.getDirectReports.mockRejectedValue(new Error('Insufficient permissions to view direct reports'));
            // Act
            const response = await request(app)
                .get(`/api/managers/${mockManager.id}/reports`)
                .set('Authorization', 'Bearer valid-token')
        ...
        it('should return 404 when manager not found', async () => {
            // Arrange
            mockEmployeeService.getDirectReports.mockRejectedValue(new Error('Manager not found'));
            // Act
            const response = await request(app)
                .get(`/api/managers/${mockManager.id}/reports`)
                .set('Authorization', 'Bearer valid-token')
        ...
        it('should handle service errors gracefully', async () => {
            // Arrange
            mockEmployeeService.getDirectReports.mockRejectedValue(new Error('Database connection failed'));
            // Act
            const response = await request(app)
                .get(`/api/managers/${mockManager.id}/reports`)
                .set('Authorization', 'Bearer valid-token')
