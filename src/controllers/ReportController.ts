...
import { ReportService, ReportFilters, ExportOptions } from '../services/ReportService';
import { ValidationError } from '../utils/validation';
import { PermissionContext } from '../services/EmployeeService';
...
export class ReportController {
   ...
   */
  async generateEmployeeRoster(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async generateDepartmentBreakdown(req: Request, res: Response): Promise<void> {
    try {
      const filters = this.parseReportFilters(req.query);
      const permissionContext = this.createPermissionContext(req.user!);
   ...
   */
  async generateWorkforceAnalytics(req: Request, res: Response): Promise<void> {
    try {
      const filters = this.parseReportFilters(req.query);
      const permissionContext = this.createPermissionContext(req.user!);
   ...
   */
  async exportEmployeeRoster(req: Request, res: Response): Promise<void> {
    try {
      const filters = this.parseReportFilters(req.query);
   ...
   */
  async exportDepartmentBreakdown(req: Request, res: Response): Promise<void> {
    try {
      const filters = this.parseReportFilters(req.query);
      const exportOptions = this.parseExportOptions(req.body);
      const permissionContext = this.createPermissionContext(req.user!);
   ...
   */
  async exportWorkforceAnalytics(req: Request, res: Response): Promise<void> {
    try {
      const filters = this.parseReportFilters(req.query);
      const exportOptions = this.parseExportOptions(req.body);
      const permissionContext = this.createPermissionContext(req.user!);
   ...
   */
  private createPermissionContext(user: any): PermissionContext {
    return {
      userId: user.id,
      role: user.role as 'HR_ADMIN' | 'MANAGER' | 'EMPLOYEE' | 'VIEWER',
   ...
   */
  private parseReportFilters(query: any): ReportFilters {
   ...
   */
  private parseExportOptions(body: any): ExportOptions {
