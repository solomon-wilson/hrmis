...
async function seedDepartments() {
    ...
    for (const dept of departments) {
        await database.query(`INSERT INTO departments (id, name, description, created_at, updated_at) 
       VALUES ($1, $2, $3, NOW(), NOW()) 
       ON CONFLICT (name) DO NOTHING`, [dept.id, dept.name, dept.description]);
    }
...
async function seedRoles() {
    ...
    for (const role of roles) {
        await database.query(`INSERT INTO roles (id, name, description, created_at, updated_at) 
       VALUES ($1, $2, $3, NOW(), NOW()) 
       ON CONFLICT (name) DO NOTHING`, [uuidv4(), role.name, role.description]);
    }
 ...
 * Seed users data
 */
async function seedUsers(_departments) {
    ...
    for (const user of users) {
        await database.query(`INSERT INTO users (id, username, email, password_hash, first_name, last_name, created_at, updated_at) 
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) 
       ON CONFLICT (email) DO NOTHING`, [user.id, user.username, user.email, user.password, user.firstName, user.lastName]);
        // Assign role to user
        const roleResult = await database.query('SELECT id FROM roles WHERE name = $1', [user.role]);
        if (roleResult.rows.length > 0) {
            await database.query(`INSERT INTO user_roles (id, user_id, role_id, created_at) 
         VALUES ($1, $2, $3, NOW()) 
         ON CONFLICT (user_id, role_id) DO NOTHING`, [uuidv4(), user.id, roleResult.rows[0].id]);
        }
    }
 ...
 * Seed employees data
 */
async function seedEmployees(users, departments) {
    ...
    for (const emp of employees) {
        await database.query(`INSERT INTO employees (
      ...
      ON CONFLICT (employee_id) DO NOTHING`, [
        ...
        await database.query(`INSERT INTO employee_status_history (
    ...
    try {
      ...
      `);
            ...
            if (hasDocs) {
                ...
                await database.query(`INSERT INTO staff_documents (
            ...
            if (hasLeave) {
                ...
                await database.query(`INSERT INTO annual_leave_plans (
...
async function clearSeedData() {
    try {
        ...
        await database.query('DELETE FROM departments WHERE name IN ($1, $2, $3, $4, $5)', ['Human Resources', 'Engineering', 'Marketing', 'Sales', 'Finance']);
        await database.query('DELETE FROM roles WHERE name IN ($1, $2, $3, $4)', ['HR_ADMIN', 'MANAGER', 'EMPLOYEE', 'VIEWER']);
